...
class MonetaryController extends CommonController{
			...
			if($hkje==($total+$znj)){
				...
				foreach ($arrs as $val) {
					...
					if($i==1){
						...
						$data['Principal']=$data['Principal']-$ybj;
					}
	...
	public function upd_money(){
		...
		if($no==$nums['num']){
			...
			// 本金余额
			if($sybj){
				$data['Principal']=$sybj-$principal;
			}else{
				$data['Principal']=$bjye-$principal;
			}
			$data['nowpri']=0;

			$datayll['nowinterest']=$data['Principal']*$yll;
			...
			if($hkje != $bjye){
				...
				//最后一期的月还款
				$nowinterest=M("cashdetails")->where("id=".max($cids))->Field("nowinterest")->find();
				$data['Arrears']=$nowinterest['nowinterest']+$data['Principal'];
			...
			M("cashinfo")->add(array("uid"=>$ids,'time'=>time(),'sum'=>$hkje,'returnlf'=>0));
			if($num){
				echo "还款成功";
			}else{
				echo "还款失败";
	...
	public function newsd(){
		...
		if($flag){
			...
			if($row){
				//修改cash期数
				M("cash")->where("id={$id}")->save(array("num"=>$endqs['num']));
				//新增一期
