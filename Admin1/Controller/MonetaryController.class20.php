...
class MonetaryController extends CommonController{
	...
	public function edit(){
		...
		//滞纳金
		$latefee=M("Cash")->Field('money,returnlaf,name,cardid,dkbh')->find($id);
		...
		$this->assign("cardid",$latefee['cardid']);
	...
	public function listCustomer(){
		...
		$data=$this->model->alias("u")->join("state s on u.state_id=s.id")->where(array("u.state_id"=>array("in",$arrs)))->Field('s.id as sid,s.name as sname,s.addtime as saddtime,s.*,u.*')->order("u.addtime desc")->select();
	...
	public function del(){
		...
		$id=$_GET['id'];
		$photo=M("applys")->where("uid={$id}")->Field('photo')->find();
	...
	public function downloadpic(){
		...
		$id=I('get.id');
		$row=M('applys')->where("uid={$id}")->find();
		...
		$arr=array($pic,$pic1);
		foreach ($arr as $val) {
			$img.="<img width=200 height=200 src=".$val.">";
		}
