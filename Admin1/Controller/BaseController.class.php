...
//		  $this->admin_member_sun = $this->sun_admin($lohb);
        ...
        //亨通
//        $lohb = array("ut.time"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
//        $lohb[] = array("ut.state_id"=>8);
//        $lohb['b.id']=1;
//        $lohb = M("uclienttime")->alias('ut')->field('ut.*')->join('__UCLIENT__ u ON u.id=ut.u_id')
//              ->join('__MEMBER__ m ON m.id=u.member_id')->join('__BRANCH__ b ON b.id=m.branch_id')
//              ->where($lohb)->select();//约见客户
//        $this->hentong = $this->sun_admin($lohb);
        //约见
        //亨通
        $lohb = array("ut.time"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $lohb[] = array("ut.state_id"=>8);
        $lohb['b.id']=1;
        $lohb = M("uclienttime")->alias('ut')->field('ut.*')->join('__ADMIN__ a ON a.id=ut.admin_id')
              ->join('__MEMBER__ m ON m.id=a.member_id')->join('__BRANCH__ b ON b.id=m.branch_id')
              ->where($lohb)->select();//约见客户
        $this->hentong = $this->sun_admin($lohb);
        ...
        $lohb = M("uclienttime")->alias('ut')->field('ut.*')->join('__ADMIN__ a ON a.id=ut.admin_id')
            ->join('__MEMBER__ m ON m.id=a.member_id')->join('__BRANCH__ b ON b.id=m.branch_id')
            ->where($lohb)->select();//约见客户
		  $this->chenghe = $this->sun_admin($lohb);
        ...
        $lohb = M("uclienttime")->alias('ut')->field('ut.*')->join('__ADMIN__ a ON a.id=ut.admin_id')
            ->join('__MEMBER__ m ON m.id=a.member_id')->join('__BRANCH__ b ON b.id=m.branch_id')
            ->where($lohb)->select();//约见客户
        $this->langyuan = $this->sun_admin($lohb);
...
//		  $this->admin_member_dan = $this->sun_admin($lohb);
        ...
        //亨通
        $lohb = array("ut.time"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $lohb[] = array("ut.state_id"=>10);
        $lohb['b.id']=1;
        $lohb = M("uclienttime")->alias('ut')->field('ut.*')->join('__ADMIN__ a ON a.id=ut.admin_id')
            ->join('__MEMBER__ m ON m.id=a.member_id')->join('__BRANCH__ b ON b.id=m.branch_id')
            ->where($lohb)->select();//约见客户
        $this->hentongqd = $this->sun_admin($lohb);
        ...
        $lohb = M("uclienttime")->alias('ut')->field('ut.*')->join('__ADMIN__ a ON a.id=ut.admin_id')
            ->join('__MEMBER__ m ON m.id=a.member_id')->join('__BRANCH__ b ON b.id=m.branch_id')
            ->where($lohb)->select();//约见客户
        $this->chengheqd = $this->sun_admin($lohb);
        ...
        $lohb = M("uclienttime")->alias('ut')->field('ut.*')->join('__ADMIN__ a ON a.id=ut.admin_id')
            ->join('__MEMBER__ m ON m.id=a.member_id')->join('__BRANCH__ b ON b.id=m.branch_id')
            ->where($lohb)->select();//约见客户
        $this->langyuanqd = $this->sun_admin($lohb);


        //放款
        //亨通
        $lohb = array("ut.time"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $lohb[] = array("ut.state_id"=>15);
        $lohb['b.id']=1;
        $lohb = M("uclienttime")->alias('ut')->field('ut.*')->join('__ADMIN__ a ON a.id=ut.admin_id')
            ->join('__MEMBER__ m ON m.id=a.member_id')->join('__BRANCH__ b ON b.id=m.branch_id')
            ->where($lohb)->select();//约见客户
        $this->hentongfk = $this->sun_admin($lohb);
        ...
        $lohb = M("uclienttime")->alias('ut')->field('ut.*')->join('__ADMIN__ a ON a.id=ut.admin_id')
            ->join('__MEMBER__ m ON m.id=a.member_id')->join('__BRANCH__ b ON b.id=m.branch_id')
            ->where($lohb)->select();//约见客户
        $this->chenghefk = $this->sun_admin($lohb);
        ...
        $lohb = M("uclienttime")->alias('ut')->field('ut.*')->join('__ADMIN__ a ON a.id=ut.admin_id')
            ->join('__MEMBER__ m ON m.id=a.member_id')->join('__BRANCH__ b ON b.id=m.branch_id')
            ->where($lohb)->select();//约见客户
        $this->langyuanfk = $this->sun_admin($lohb);

        //录入
        //亨通
        $lohb = array("ut.addtime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        // $lohb[] = array("ut.state_id"=>8);
        $lohb['b.id']=1;
        $lohb = M("uclient")->alias('ut')->field('ut.*')->join('__ADMIN__ a ON a.id=ut.admin_id')
            ->join('__MEMBER__ m ON m.id=a.member_id')->join('__BRANCH__ b ON b.id=m.branch_id')
            ->where($lohb)->select();//录入
      //  var_dump(count($lohb));
        $this->hentonglr = count($lohb);
      ...
      //  var_dump(count($lohb));
        ...
        $lohb = M("uclient")->alias('ut')->field('ut.*')->join('__ADMIN__ a ON a.id=ut.admin_id')
            ...
            ->where($lohb)->select();//录入
     //   var_dump(count($lohb));
        $this->langyuanlr = count($lohb);

        //首页统计——宝安1部
        //速分贷
        $wherey=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $wherey[]=array("branch"=>array('in',array(16,17)),'type'=>1);
        $bayb=M("cash")->where(array("branch"=>array('in',array(16,17)),'type'=>1))->sum("money");
        $baybs=M("cash")->where($wherey)->sum("money");
        $bayb=$bayb/10000;
        $bayb=substr(sprintf("%.3f",$bayb),0,-1);
        $baybs=$baybs/10000;
        $baybs=substr(sprintf("%.3f",$baybs),0,-1);
        $this->assign('zybsum',$bayb);
        $this->assign('ybsum',$baybs);
        //速贷
        $whereysd=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $whereysd[]=array("branch"=>array('in',array(16,17)),'type'=>2);
        $bayb=M("cash")->where(array("branch"=>array('in',array(16,17)),'type'=>2))->sum("money");
        $baybs=M("cash")->where($whereysd)->sum("money");
        
        $bayb=$bayb/10000;
        $bayb=substr(sprintf("%.3f",$bayb),0,-1);
        $baybs=$baybs/10000;
        $baybs=substr(sprintf("%.3f",$baybs),0,-1);
        $this->assign('zybsum1',$bayb);
        $this->assign('ybsum1',$baybs);

        //宝安2部
        //速分贷
        $wheree=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $wheree[]=array("branch"=>array('in',array(18,19)),'type'=>1);
        $baeb=M("cash")->where(array("branch"=>array('in',array(18,19)),'type'=>1))->sum("money");
        $baebs=M("cash")->where($wheree)->sum("money");
        $baeb=$baeb/10000;
        $baeb=substr(sprintf("%.3f",$baeb),0,-1);
        $baebs=$baebs/10000;
        $baebs=substr(sprintf("%.3f",$baebs),0,-1);
        $this->assign('zebsum',$baeb);
        $this->assign('ebsum',$baebs);
        //速贷
        $whereesd=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $whereesd[]=array("branch"=>array('in',array(18,19)),'type'=>2);
        $baeb=M("cash")->where(array("branch"=>array('in',array(18,19)),'type'=>2))->sum("money");
        $baebs=M("cash")->where($whereesd)->sum("money");
        $baeb=$baeb/10000;
        $baeb=substr(sprintf("%.3f",$baeb),0,-1);
        $baebs=$baebs/10000;
        $baebs=substr(sprintf("%.3f",$baebs),0,-1);
        $this->assign('zebsum1',$baeb);
        $this->assign('ebsum1',$baebs);


         //现金1部
        //速分贷
        $where=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $where[]=array("branch"=>array('in',array(30,29)),'type'=>1);
        $xjyb=M("cash")->where(array("branch"=>array('in',array(30,29)),'type'=>1))->sum("money");
        $xjybs=M("cash")->where($where)->sum("money");
        $xjyb=$xjyb/10000;
        $xjyb=substr(sprintf("%.3f",$xjyb),0,-1);
        $xjybs=$xjybs/10000;
        $xjybs=substr(sprintf("%.3f",$xjybs),0,-1);
        $this->assign('xjybsum',$xjyb);
        $this->assign('xjybsums',$xjybs);
        //速贷
        $where=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $where[]=array("branch"=>array('in',array(30,29)),'type'=>2);
        $xjyb=M("cash")->where(array("branch"=>array('in',array(30,29)),'type'=>2))->sum("money");
        $xjybs=M("cash")->where($where)->sum("money");
        $xjyb=$xjyb/10000;
        $xjyb=substr(sprintf("%.3f",$xjyb),0,-1);
        $xjybs=$xjybs/10000;
        $xjybs=substr(sprintf("%.3f",$xjybs),0,-1);
        $this->assign('xjybsum1',$xjyb);
        $this->assign('xjybsums1',$xjybs);

         //现金2部
        //速分贷
        $where=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $where[]=array("branch"=>array('in',array(7,8)),'type'=>1);
        $xjeb=M("cash")->where(array("branch"=>array('in',array(7,8)),'type'=>1))->sum("money");
        $xjebs=M("cash")->where($where)->sum("money");
        $xjeb=$xjeb/10000;
        $xjeb=substr(sprintf("%.3f",$xjeb),0,-1);
        $xjebs=$xjebs/10000;
        $xjebs=substr(sprintf("%.3f",$xjebs),0,-1);
        $this->assign('xjebsum',$xjeb);
        $this->assign('xjebsums',$xjebs);
        //速贷
        $where=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $where[]=array("branch"=>array('in',array(7,8)),'type'=>2);
        $xjeb=M("cash")->where(array("branch"=>array('in',array(7,8)),'type'=>2))->sum("money");
        $xjebs=M("cash")->where($where)->sum("money");
        $xjeb=$xjeb/10000;
        $xjeb=substr(sprintf("%.3f",$xjeb),0,-1);
        $xjebs=$xjebs/10000;
        $xjebs=substr(sprintf("%.3f",$xjebs),0,-1);
        $this->assign('xjebsum1',$xjeb);
        $this->assign('xjebsums1',$xjebs);

          //现金3部
        //速分贷
        $where=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $where[]=array("branch"=>array('in',array(25,9)),'type'=>1);
        $xjsb=M("cash")->where(array("branch"=>array('in',array(25,9)),'type'=>1))->sum("money");
        $xjsbs=M("cash")->where($where)->sum("money");
        $xjsb=$xjsb/10000;
        $xjsb=substr(sprintf("%.3f",$xjsb),0,-1);
        $xjsbs=$xjsbs/10000;
        $xjsbs=substr(sprintf("%.3f",$xjsbs),0,-1);
        $this->assign('xjsbsum',$xjsb);
        $this->assign('xjsbsums',$xjsbs);
        //速贷
        $where=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $where[]=array("branch"=>array('in',array(25,9)),'type'=>2);
        $xjsb=M("cash")->where(array("branch"=>array('in',array(25,9)),'type'=>2))->sum("money");
        $xjsbs=M("cash")->where($where)->sum("money");
        $xjsb=$xjsb/10000;
        $xjsb=substr(sprintf("%.3f",$xjsb),0,-1);
        $xjsbs=$xjsbs/10000;
        $xjsbs=substr(sprintf("%.3f",$xjsbs),0,-1);
        $this->assign('xjsbsum1',$xjsb);
        $this->assign('xjsbsums1',$xjsbs);

          //现金4部
        //速分贷
        $where=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $where[]=array("branch"=>33,'type'=>1);
        $xj4b=M("cash")->where(array("branch"=>33,'type'=>1))->sum("money");
        $xj4bs=M("cash")->where($where)->sum("money");
        $xj4b=$xj4b/10000;
        $xj4b=substr(sprintf("%.3f",$xj4b),0,-1);
        $xj4bs=$xj4bs/10000;
        $xj4bs=substr(sprintf("%.3f",$xj4bs),0,-1);
        $this->assign('xj4bsum',$xj4b);
        $this->assign('xj4bsums',$xj4bs);
        //速贷
        $where=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $where[]=array("branch"=>33,'type'=>2);
        $xj4b=M("cash")->where(array("branch"=>33,'type'=>2))->sum("money");
        $xj4bs=M("cash")->where($where)->sum("money");
        $xj4b=$xj4b/10000;
        $xj4b=substr(sprintf("%.3f",$xj4b),0,-1);
        $xj4bs=$xj4bs/10000;
        $xj4bs=substr(sprintf("%.3f",$xj4bs),0,-1);
        $this->assign('xj4bsum1',$xj4b);
        $this->assign('xj4bsums1',$xj4bs);
...
//		  $this->admin_member_dans = $this->sun_admin($lohbs);
	    ...
	    if(in_array('1002',$_SESSION['level'])){
...
//              var_dump($this->totalSign);exit;
        ...
        //查看是否顶级管理员
        $aid=M('admin')->where(['id'=>session('aid')])->getField('level_id');
        $pid=M('rank')->where(['id'=>$aid])->getField('pid');
        if ($pid==0){
            $autograph = M("uclient")->getField("id",true);
		...
		$this->monthall = $this->qudao(array("state_id"=>10),$autograph,false,$is_manager);//今日签约
            var_dump($this->monthall);
	...
	//旧版
           ...
           //8.30fitime改time
            $monthsun = array("time"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
 ...
 //旧版
        //$monthprice = array("fitime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
            //8.30fitime改time
            $monthprice = array("time"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
