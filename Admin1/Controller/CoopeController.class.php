...
class CoopeController extends CommonController{
    ...
    public function index()
    {
                        ...
                        if($post['sou']){
                            ...
                            if($sou_member){
                                ...
                                if($sou_state || $post['sou'] == iconv("GB2312","UTF-8","未联系")){
                                    ...
                                    // $where['username']=array('like','%'.$post['sou'].'%');
                                    $where[]["username|phone"] = array('like','%'.$post['sou'].'%');
                                    $where[] = array("state_id"=>array("in",$state));
                                }
                                $where[] = $kun;
                            }
        ...
        if($post['stime'] && $post['endtime'] && $post['sou'] && $post['khlx'] && $post['gjzt']){
            $sou_member=M("member")->where(array("username"=>$post['sou']))->find();
            if($sou_member){
                $sou_members=M("admin")->where(array("member_id"=>$sou_member['id']))->getField("id");
            }
            if($sou_members){
                $where = array("admin_id"=>$sou_members,"state_id"=>$post['gjzt'],"type_id"=>$post['khlx'],"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
                $where[] = $kun;
            }else{
                $where["username|phone"] = array('like','%'.$post['sou'].'%');

                $where[] = array("state_id"=>$post['gjzt'],"type_id"=>$post['khlx'],"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
                $where[] = $kun;

            }
        }elseif($post['stime'] && $post['endtime'] && $post['branch'] && $post['khlx'] && $post['gjzt']){
            $aids=M("admin")->where("branch_id={$post['branch']}")->getField("id",true);
            $where = array("admin_id"=>array("in",$aids),"state_id"=>$post['gjzt'],"type_id"=>$post['khlx'],"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
            $where[] = $kun;

        }elseif($post['stime'] && $post['endtime'] && $post['branch'] && $post['khlx']){
            $aids=M("admin")->where("branch_id={$post['branch']}")->getField("id",true);
            $where = array("admin_id"=>array("in",$aids),"type_id"=>$post['khlx'],"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
            $where[] = $kun;

        }elseif($post['stime'] && $post['endtime'] && $post['branch'] && $post['gjzt']){
            $aids=M("admin")->where("branch_id={$post['branch']}")->getField("id",true);
            $where = array("admin_id"=>array("in",$aids),"state_id"=>$post['gjzt'],"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
            $where[] = $kun;

        }elseif($post['stime'] && $post['endtime'] && $post['branch']){
            $aids=M("admin")->where("branch_id={$post['branch']}")->getField("id",true);
            $where = array("admin_id"=>array("in",$aids),"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
            $where[] = $kun;

        }elseif($post['branch'] && $post['khlx'] && $post['gjzt']){
            $aids=M("admin")->where("branch_id={$post['branch']}")->getField("id",true);
            $where = array("admin_id"=>array("in",$aids),"state_id"=>$post['gjzt'],"type_id"=>$post['khlx']);
            $where[] = $kun;

        }elseif($post['branch'] && $post['gjzt']){
            $aids=M("admin")->where("branch_id={$post['branch']}")->getField("id",true);
            $where = array("admin_id"=>array("in",$aids),"state_id"=>$post['gjzt']);
            $where[] = $kun;

        }elseif($post['branch'] && $post['khlx']){
            $aids=M("admin")->where("branch_id={$post['branch']}")->getField("id",true);
            $where = array("admin_id"=>array("in",$aids),"type_id"=>$post['khlx']);
            $where[] = $kun;

        }elseif($post['stime'] && $post['endtime'] && $post['sou'] && $post['khlx']){
            $sou_member=M("member")->where(array("username"=>$post['sou']))->find();
            if($sou_member){
                $sou_members=M("admin")->where(array("member_id"=>$sou_member['id']))->getField("id");
            }
            if($sou_members){
                $where = array("admin_id"=>$sou_members,"type_id"=>$post['khlx'],"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
                $where[] = $kun;
            }else{
                $where["username|phone"] = array('like','%'.$post['sou'].'%');

                $where[] = array("type_id"=>$post['khlx'],"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
                $where[] = $kun;

            }
        }elseif($post['stime'] && $post['endtime'] && $post['sou'] && $post['gjzt']){
            $sou_member=M("member")->where(array("username"=>$post['sou']))->find();
            if($sou_member){
                $sou_members=M("admin")->where(array("member_id"=>$sou_member['id']))->getField("id");
            }
            if($sou_members){
                $where = array("admin_id"=>$sou_members,"state_id"=>$post['gjzt'],"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
                $where[] = $kun;
            }else{
                $where["username|phone"] = array('like','%'.$post['sou'].'%');

                $where[] = array("state_id"=>$post['gjzt'],"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
                $where[] = $kun;

            }
        }elseif($post['stime'] && $post['endtime'] && $post['khlx'] && $post['gjzt']){
            $where = array("state_id"=>$post['gjzt'],"type_id"=>$post['khlx'],"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
            $where[] = $kun;
            ...
            if($sou_members){
                $where = array("admin_id"=>$sou_members,"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
                $where[] = $kun;
            }else{
                $where["username|phone"] = array('like','%'.$post['sou'].'%');

                $where[] = array("lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
                $where[] = $kun;

            }
        }elseif($post['stime'] && $post['endtime'] && $post['khlx']){
            $where = array("type_id"=>$post['khlx'],"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
            $where[] = $kun;
        }elseif($post['stime'] && $post['endtime'] && $post['gjzt']){
            $where = array("state_id"=>$post['gjzt'],"lxtime"=>array(array("gt",$stime),array('elt',$endtime)));
            $where[] = $kun;

        }elseif($post['sou'] && $post['khlx'] && $post['gjzt']){
            $sou_member=M("member")->where(array("username"=>$post['sou']))->find();
            if($sou_member){
                $sou_members=M("admin")->where(array("member_id"=>$sou_member['id']))->getField("id");
            }
            if($sou_members){
                $where = array("admin_id"=>$sou_members,"state_id"=>$post['gjzt'],"type_id"=>$post['khlx']);
                $where[] = $kun;
            }else{
                $where["username|phone"] = array('like','%'.$post['sou'].'%');
                $where[] = array("state_id"=>$post['gjzt'],"type_id"=>$post['khlx']);
                $where[] = $kun;
            }
        }elseif($post['sou'] && $post['khlx']){
            $sou_member=M("member")->where(array("username"=>$post['sou']))->find();
            if($sou_member){
                $sou_members=M("admin")->where(array("member_id"=>$sou_member['id']))->getField("id");
            }
            if($sou_members){
                $where = array("admin_id"=>$sou_members,"type_id"=>$post['khlx']);
                $where[] = $kun;
            }else{
                $where["username|phone"] = array('like','%'.$post['sou'].'%');
                $where[] = array("type_id"=>$post['khlx']);
                $where[] = $kun;
            }
            ...
            if($sou_members){
                $where = array("admin_id"=>$sou_members,"state_id"=>$post['gjzt']);
                $where[] = $kun;
            }else{
                $where["username|phone"] = array('like','%'.$post['sou'].'%');
                $where[] = array("state_id"=>$post['gjzt']);
                $where[] = $kun;
            }
            ...
            $aids=M("admin")->where("branch_id={$post['branch']}")->getField("id",true);
            $where = array("admin_id"=>array("in",$aids));
            $where[] = $kun;
