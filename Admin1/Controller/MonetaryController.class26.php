...
class MonetaryController extends CommonController{
	...
	public function upd_money(){
		...
		$data['Principal']=$bjye-$hkje;
		$data['nowpri']=0;
		$data['nowinterest']=$data['Principal']*$yll;
		$data['Arrears']=$data['nowinterest'];
		
		//修改还款金额
		M("cashdetails")->where(array('num'=>$no,'uid'=>$ids))->save(array("returnpri"=>$hkje));
		...
		if($hkje != $bjye){
			...
			$data['Arrears']=$data['nowinterest']+$data['Principal'];
