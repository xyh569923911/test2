...
//         var_dump($this->ch_data);
        foreach ($this->ch_data as $k=> $val) {
//            $v=implode(",", $val);
            $v=implode("!!##", $val);
            $arr[$k]=$v;
        }
...
//            $array=explode(",", $v);
           ...
           /* if($array[1]=="王志祥"){
            ...
            if(count($array)==21){
                $arrs[$k]["id"] = $array[0];
                $arrs[$k]["username"] = $array[1];
                $arrs[$k]["password"] = $array[2];
                $arrs[$k]["level_id"] = $array[3];
                $arrs[$k]["addtime"] = $array[4];
                $arrs[$k]["dladdtime"] = $array[5];
                $arrs[$k]["vmember_id"] = $array[6];
                $arrs[$k]["passw"] = $array[7];
                $arrs[$k]["branch_id"] = $array[8];
                $arrs[$k]["sort"] = $array[9];
                $arrs[$k]["type"] = $array[10];
                $arrs[$k]["dow"] = $array[11];
                $arrs[$k]["types"] = $array[12];
                $arrs[$k]["name"] = $array[13];
                $arrs[$k]["cid"] = $array[14];
                $arrs[$k]["pid"] = $array[15];
                $arrs[$k]["_level"] = $array[16];
                $arrs[$k]["_html"] = $array[17];
                $arrs[$k]["_first"] = $array[18];
                $arrs[$k]["_end"] = $array[19];
                $arrs[$k]["_name"] = $array[20];
            }else{
                $arrs[$k]["id"] = $array[0];
                $arrs[$k]["username"] = $array[1];
                $arrs[$k]["password"] = $array[2];
        ...
        foreach ($arrs as $key => $val) {
            ...
            if($val['id']==72){
                ...
                array_splice($arr,$keys+1,0,array($val));
                foreach ($arr as $k => $v) {
                    if($v['id']==72){
                        $key=$k;
                    }
    ...
    public function delById()
    {
        ...
        if($m = ucfirst(I("post.m", "")))
        {
            ...
            if($arr=$model->find($id))
            {
                ...
                //查找管理员有没有此人
                $srt = M("uclient")->where(array("admin_id"=>$id,'_logic'=>'or',"finance_admin_id"=>$id,"rank_admin_id"=>$id))->find();
                $applys=M("applys")->where("salesman=".$id)->find();
                $cash=M("cash")->where("Salesman=".$id)->find();
                $carinfo=M("carinfo")->where("salesman=".$id)->find();
                $carloan=M("carloan")->where("salesmans=".$id)->find();

                if(!$srt && !$applys && !$cash && !$carinfo && !$carloan){
                    $model->where(array("id" => $id))->delete();
                    M("admin_sms")->where(array("admin_id" => $id))->delete();
                    echo 1;
                }else{
