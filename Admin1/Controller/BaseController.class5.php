...
//		  $this->admin_member_dan = $this->sun_admin($lohb);
     ...
     //   var_dump(count($lohb));
        ...
        $bayb=M("cash")->where(array("branch"=>array('in',array(16,17))))->field("money")->select();
        $baybs=M("cash")->where($where)->field("money")->select();
        foreach ($bayb as $k => $v) {
        	$zybsum+=$v['money'];
        }
        foreach ($baybs as $k => $v) {
        	$ybsum+=$v['money'];
        }
        $this->assign('zybsum',$zybsum);
        $this->assign('ybsum',$ybsum);

        //宝安2部
        $where=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $where[]=array("branch"=>array('in',array(18,19)));
        $baeb=M("cash")->where(array("branch"=>array('in',array(18,19))))->field("money")->select();
        $baebs=M("cash")->where($where)->field("money")->select();
        foreach ($baeb as $k => $v) {
        	$zebsum+=$v['money'];
        }
        foreach ($baebs as $k => $v) {
        	$ebsum+=$v['money'];
        }
        $this->assign('zebsum',$zebsum);
        $this->assign('ebsum',$ebsum);

         //现金1部
        $where=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $where[]=array("branch"=>array('in',array(30,29)));
        $xjyb=M("cash")->where(array("branch"=>array('in',array(30,29))))->field("money")->select();
        $xjybs=M("cash")->where($where)->field("money")->select();
        foreach ($xjyb as $k => $v) {
        	$xjybsum+=$v['money'];
        }
        foreach ($xjybs as $k => $v) {
        	$xjybsums+=$v['money'];
        }
        $this->assign('xjybsum',$xjybsum);
        $this->assign('xjybsums',$xjybsums);

         //现金2部
        $where=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $where[]=array("branch"=>array('in',array(7,8)));
        $xjeb=M("cash")->where(array("branch"=>array('in',array(7,8))))->field("money")->select();
        $xjebs=M("cash")->where($where)->field("money")->select();
        foreach ($xjeb as $k => $v) {
        	$xjebsum+=$v['money'];
        }
        foreach ($xjebs as $k => $v) {
        	$xjebsums+=$v['money'];
        }
        $this->assign('xjebsum',$xjebsum);
        $this->assign('xjebsums',$xjebsums);

          //现金3部
        $where=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $where[]=array("branch"=>array('in',array(25,9)));
        $xjsb=M("cash")->where(array("branch"=>array('in',array(25,9))))->field("money")->select();
        $xjsbs=M("cash")->where($where)->field("money")->select();
        foreach ($xjsb as $k => $v) {
        	$xjsbsum+=$v['money'];
        }
        foreach ($xjsbs as $k => $v) {
        	$xjsbsums+=$v['money'];
        }
        $this->assign('xjsbsum',$xjsbsum);
        $this->assign('xjsbsums',$xjsbsums);

          //现金4部
        $where=array("starttime"=>array(array('gt',strtotime(date('Y-m-01 0:00:01'))),array("elt",mktime(23,59,59,date('m'),date('t'),date('Y')))));
        $where[]=array("branch"=>33);
        $xj4b=M("cash")->where(array("branch"=>33))->field("money")->select();
        $xj4bs=M("cash")->where($where)->field("money")->select();
        foreach ($xj4b as $k => $v) {
        	$xj4bsum+=$v['money'];
        }
        foreach ($xj4bs as $k => $v) {
        	$xj4bsums+=$v['money'];
        }
        $this->assign('xj4bsum',$xj4bsum);
        $this->assign('xj4bsums',$xj4bsums);
