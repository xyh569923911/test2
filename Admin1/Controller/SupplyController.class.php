...
class SupplyController extends CommonController{
	...
	{   //header("Content-type: text/html; charset=utf-8"); 
	   if(in_array('1001',$_SESSION['level'])){
		  ...
		  $ids[]=$id;
		  //var_dump($ids);
		  $kun = array("state"=>1,"admin_ids"=>array("in",$ids));
		//  print_r($kun);exit;
	...
	//	echo $this->model->getLastSql();exit;
	 ...
	 /*  if(in_array('1001',$_SESSION['level'])){
		...
		$count=count($ch_datatss);//得到数组元素个数
		$Page= $this->getPage($count,45);// 实例化分页类 传入总记录数和每页显示的记录数
		$ch_datatss = array_slice($ch_datatss,$Page->firstRow,$Page->listRows);
	...
	public function editindex(){
		...
		$this->assign("genjin", $genjin);
		 ...
		 $this->assign("columnsss", $columnsss);
		 //所属
